...
public class Cell
{
    ...
    [Space]
    public Node neighbourUp;
    public Node neighbourDown;
    public Node neighbourLeft;
    public Node neighbourRight;
    ...
    public void SetNeighbours(Cell[,] grid)
    {
        ...
        if (row + 1 < grid.GetLength(0))
        {
            if (currentCell.HasWall(Wall.UP) == false)
                neighbourUp = new Node(new Vector2Int(grid[col, row + 1].gridPosition.x, grid[col, row + 1].gridPosition.y), null, 9999, 9999);
        }
        if (row - 1 >= 0)
        {
            if (currentCell.HasWall(Wall.DOWN) == false)
                neighbourDown = new Node(new Vector2Int(grid[col, row - 1].gridPosition.x, grid[col, row - 1].gridPosition.y), null, 9999, 9999);
        }
        if (col - 1 >= 0)
        {
            if (currentCell.HasWall(Wall.LEFT) == false)
                neighbourLeft = new Node(new Vector2Int(grid[col - 1, row].gridPosition.x, grid[col - 1, row].gridPosition.y), null, 9999, 9999);
        }
        if (col + 1 < grid.GetLength(1))
        {
            if (currentCell.HasWall(Wall.RIGHT) == false)
                neighbourRight = new Node(new Vector2Int(grid[col + 1, row].gridPosition.x, grid[col + 1, row].gridPosition.y), null, 9999, 9999);
        }
    }
